...
-- DITA help topics imported from chatbot/ChatbotHilfe (or any other
-- DITA-OT XHTML output). One row per (topic, language) — sys_language_uid
-- + l10n_parent so EN/FR translations imported from sibling folders chain
-- back to the default-language source.
-- The `identifier` is the DITA DC.identifier and is the natural key
-- within a single language; helpDocSchemaProvider builds document ids of
-- the form "help-<uid>" (and "help-<uid>-l<n>" for translations) so they
-- align with how FileSchemaProvider handles multilingual docs.
CREATE TABLE tx_wsmeilisearch_helpdoc (
    ...
    -- Primary media (image / video) extracted from the topic by the
    -- importer and copied into fileadmin/chatbot-hilfe/<identifier>/.
